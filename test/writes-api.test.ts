...
describe('/api/ 書き込みAPI', () => {
  ...
  it('トークン無し・不正トークンは401', async () => {
    expect((await rw('/api/menus', null, 'POST', { name: 'x', calories: 1 })).status).toBe(401);
    expect((await rw('/api/menus', 'bogus', 'POST', { name: 'x', calories: 1 })).status).toBe(401);
  });

  it('メニューの作成・更新・アーカイブができる', async () => {
    const created = await rw('/api/menus', token, 'POST', { name: '牛丼', calories: 700, protein_g: 20 });
    expect(created.status).toBe(201);
    const menu = (await created.json()) as { id: string };

    const patched = await rw(`/api/menus/${menu.id}`, token, 'PATCH', { name: '牛丼大盛', calories: 900 });
    ...
    expect((await rw(`/api/menus/${menu.id}/archive`, token, 'POST')).status).toBe(200);
    expect((await rw(`/api/menus/${menu.id}/unarchive`, token, 'POST')).status).toBe(200);
  });

  it('メニューPATCHは真の部分更新: 未指定は保持、明示nullはクリア、空パッチは400', async () => {
    const created = await rw('/api/menus', token, 'POST', {
      name: '定食', calories: 800, protein_g: 25, fat_g: 15, carbs_g: 90, note: 'メモ',
    });
    const menu = (await created.json()) as { id: string };

    // nameだけ送る → calories/protein_g/fat_g/carbs_g/noteは保持される
    const onlyName = await rw(`/api/menus/${menu.id}`, token, 'PATCH', { name: '定食大盛' });
    ...
    const cleared = await rw(`/api/menus/${menu.id}`, token, 'PATCH', { protein_g: null });
    ...
    expect((await rw(`/api/menus/${menu.id}`, token, 'PATCH', {})).status).toBe(400);

    // 本文がオブジェクトでない妥当なJSON（数値・文字列・配列）も400（500にならないこと）
    expect((await rw(`/api/menus/${menu.id}`, token, 'PATCH', 5)).status).toBe(400);
    expect((await rw(`/api/menus/${menu.id}`, token, 'PATCH', 'x')).status).toBe(400);
    expect((await rw(`/api/menus/${menu.id}`, token, 'PATCH', ['x'])).status).toBe(400);
  });

  it('バリデーション: caloriesが負・multiplier過大・不正meal_typeは400', async () => {
    expect((await rw('/api/menus', token, 'POST', { name: 'x', calories: -1 })).status).toBe(400);
    const menu = await createMenu(testEnv, { name: 'a', calories: 100 });
    expect((await rw('/api/meals', token, 'POST', { menu_id: menu.id, multiplier: 100 })).status).toBe(400);
    expect((await rw('/api/meals', token, 'POST', { menu_id: menu.id, meal_type: 'brunch' })).status).toBe(400);
  });

  it('記録の作成（201）→修正→削除。archivedメニューへの記録は400', async () => {
    const menu = await createMenu(testEnv, { name: 'b', calories: 500 });
    const res = await rw('/api/meals', token, 'POST', { menu_id: menu.id, multiplier: 1.5, meal_type: 'lunch' });
    expect(res.status).toBe(201);
    const log = (await res.json()) as { id: string; effective_calories: number };
    expect(log.effective_calories).toBeCloseTo(750);

    expect((await rw(`/api/meals/${log.id}`, token, 'PATCH', { multiplier: 1 })).status).toBe(200);
    expect((await rw(`/api/meals/${log.id}`, token, 'DELETE')).status).toBe(200);
    expect((await rw(`/api/meals/${log.id}`, token, 'DELETE')).status).toBe(404);
