...
| `POST /mcp` | MCP（Model Context Protocol）エンドポイント。OAuth 認証必須。読み取り8ツール + 書き込み6ツール（`log_meal` `create_menu` `log_exercise` `create_exercise_menu` `set_goal` `log_weight`） |
