...
| `GET {base}/api/exercise/records?menu_id=` | 筋トレ種目の自己ベスト（最大重量・REP数ごとの最大重量・推定1RM・最大REP・最大セット/セッションボリューム・前回セッション）。都度集計で DB には持たない。`menu_id` 無しは 400、有酸素種目は 400。認証不要 |
...
- **フォールバック: GitHub の schedule（23:30 / 23:50 JST の2本）** — `coaching.yml` に残る（GitHub は schedule を遅らせるだけでなく発火自体を落とすことがあるため2本立て）。「その夜のスロット（23:30）以降に生成された講評」が既に保存されていればスキップし（未明の遅延実行や日中の手動再生成が残した古い講評は上書きする。Worker 起動分の生成完了前に schedule 側が走った場合は二重生成になりうるが、同内容の upsert で実害はない）、遅延で日付をまたいで起動した場合は前日分（本来の対象日）を生成する。PAT を設定しない場合はこの経路だけで従来どおり動く
...
- GitHub の schedule は数分〜十数分（まれに数時間）遅れるほか、発火自体をスキップすることもあり、23:55 のダイジェストに生成が間に合わなかった日は数値のみで配信される（講評はダッシュボードには出る）。遅延が日付をまたいだ場合も、当日分ではなく本来の対象日（前日）の講評として生成される。Worker からの起動（上記）を設定していれば、この不安定さの影響は実質受けない
- 日次ダイジェストは 23:55 時点でその日の記録（体重・食事・運動）が何も無いとスキップされ、日付が変わった後は自動では送られない。記録を後から足した日は、Actions の「Send Daily Digest」（`.github/workflows/digest.yml`）を `date` 指定で Run workflow すると `POST /api/digest` 経由で送り直せる。講評を差し込みたければ先に「AI Coaching」を同じ `date` で実行し、**その完了を待ってから**送る（2つのワークフローは別の concurrency group で直列化されない）。一度送った日は送信済みとして 409 になり、講評を後から差し込み直すことはできない
