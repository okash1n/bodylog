...
> **後続の変更**: MCP は食事記録 Phase 1（`2026-08-12-meal-tracking-phase1-design.md`）以降 OAuth 2.1 認可必須になり（無認証の公開 `/mcp` は廃止）、ツールは読み取り 8 ＋書き込み 14 に増えた。読み取り REST の公開/非公開は `READ_ACCESS`（`2026-08-20-read-access-control-design.md`）で選べる。以下の「認証なし」「読み取り専用の3ツール」は当時の設計。
