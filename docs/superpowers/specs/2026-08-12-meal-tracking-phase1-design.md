...
> **後続の変更**: 認証必須パスの `/rw/` プレフィックスと「公開 `/mcp` ＋認証付き `/rw/mcp`」の二本立ては廃止された。現行は、書き込みが読み取りと同じ `{base}/api/*` パスにメソッドで同居しハンドラごとに Bearer を検証、MCP は `/mcp` の単一エンドポイント（OAuth 必須。読み取りツールも認証下）。読み取り REST の公開/非公開は `READ_ACCESS`（`2026-08-20-read-access-control-design.md`）で選べる。以下の `/rw/` に関する記述は当時の設計。
